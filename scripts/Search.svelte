<script lang="ts">
  ...
  let queryStack: [string | undefined, string | undefined] = [
    undefined,
    undefined,
  ];
  ...
  function handleKeydown(event: KeyboardEvent) {
    if (event.code === "ArrowUp" && composers.length > 0) {
      selectedResult =
        selectedResult > 0 ? selectedResult - 1 : composers.length - 1;
...
{#if searchVisible}
  <div class="search-wrapper" on:keydown={handleKeydown}>
    <div class="search" use:clickOutside={hideSearch}>
      <input
        class="search__field"
        type="search"
      ...
      {#each composers as composer, i}
        ...
        >
          <div
            class:search__result_selected={selectedResult === i}
